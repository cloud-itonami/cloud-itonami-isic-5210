(ns terminal.operation
  "OperationActor -- one terminal-storage operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (TerminalAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Terminal Storage Governor (:govern) and the rollout phase gate
  ...
  One graph run = one terminal-storage operation (intake -> advise -> govern ->
  decide -> commit | hold | approval). No unbounded inner loop -- each
  operation is auditable and checkpointed. A tank's life is advanced by
  MANY ops (intake / verify-receipt / commit-storage / transfer-custody),
  ...
  decision to a human operator (terminal operator / depot superintendent).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:storage/commit`/`:custody/transfer` ALWAYS reach this node
  when the governor is clean -- see `terminal.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [terminal.terminaladvisor :as terminaladvisor]
...
(defn build
  ...
  opts:
    :advisor      -- a `terminal.terminaladvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (terminaladvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TerminalAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (terminaladvisor/-advise advisor store request)]
            {:proposal p :audit [(terminaladvisor/trace request p)]})))

      ;; Terminal Storage Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
